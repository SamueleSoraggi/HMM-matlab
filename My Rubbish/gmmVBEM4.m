function [out] = gmmVBEM4(X, gmm_obj, PriorPar, options)

% This version of VBGMMEM is adpated from Emtiyaz's original version. The
% only difference between this version and the original is that this
% version uses GMM object from MATLAB as opposed to netlab3_3. This might
% be more convenient for the users. The rest of the code are the same. If
% you found the code useful, please thank Emtiyaz, CS, UBC instead of me.
% =============
% Kittipat Kampa
% Aug 8, 2011
...
[D N] = size(x);
K = gmm_obj.NComponents; % mix.ncentres;
...
% CB 10.51-10.53.
Nk = N*gmm_obj.PComponents(:); % N*mix.priors';
xbar = gmm_obj.mu'; % mix.centres';
S = gmm_obj.Sigma; % mix.covars;
...
%valids = find(sum(m.^2-repmat(m0,1,K).^2,2) > 1e-5)
...
if options.displayFig==1
     ...
     if D==2
        clf(figura);        
        hold on
        plot(x(:,1),x(:,2),'Color',[0 .1 .9],'LineStyle','*','LineWidth',2)
        for i = 1:K
               % if any(i==valids)
        ...
        hold off
        s1 = sprintf('Gmm (proiettato) all''iterata %d',iter);
        s2 = sprintf('\nLowerBound = %f',L(iter));
        s3 = sprintf('\n%d stati nascosti',Q);
        ...
        clf(figura); 
        hold on
        plot(x(2,:),x(3,:),'Color',[0 .1 .9],'LineStyle','*','LineWidth',2)
        for i = 1:K
                %if any(i==valids)
                SS = inv(W(:,:,i))/(v(i)-D-1); SS = SS([2 3],[2 3]);
                M = m(:,i); M = M([2,3]);
                MyEllipse(SS, M,'style','r','intensity',weight(i), 'facefill',.8);
